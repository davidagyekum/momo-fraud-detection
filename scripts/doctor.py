...
def collect_checks() -> list[Check]:
    ...
    checks = [
        Check(
            ...
            + ("" if python_ok else " (Python 3.12 is required; use py -3.12 on Windows)"),
            "P00+",
        )
    ]
    ...
    definitions = [
        ...
        ("npm", ["npm.cmd" if platform.system() == "Windows" else "npm", "--version"], "P04/P05", False),
...
def main() -> int:
    ...
    if args.json:
        ...
        for check in checks:
            print(f"[{check.status:7}] {check.name}: {check.detail} (needed: {check.required_for})")
    ...
    print(f"Doctor result: PASS for P00; {len(missing)} later-phase tool(s) not installed")
    return 0
