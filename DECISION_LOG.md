...
## ADR-036 — Require complete OCR comparison coverage and exact runtime compatibility

- **Date:** 2026-08-14
- **Status:** Accepted
- **Decision owners:** Codex/model steward under ADR-035 and the measured failed validation run
- **Context:** The first completed clean-validation pass produced a syntactically valid v1 report, but `field_region` existed for only eight of 33 records and nevertheless became the EasyOCR/Tesseract finalist. PaddleOCR failed all 33 records because PaddleOCR 3.7 enabled MKLDNN by default and PaddlePaddle 3.3.1 could not convert a PP-OCRv6 oneDNN PIR array attribute. Ubuntu Jammy supplied Tesseract 4.1.1 even though the benchmark contract requires major 5. The v1 selector still wrote an `experimental` EasyOCR bundle, which could be mistaken for a comparable winner despite incomplete coverage.
- **Options considered:** Keep partial configurations and weight by coverage; relax the Tesseract version to 4; omit PaddleOCR; accept a third-party PPA; require exact complete coverage and repair each runtime at its supported boundary.
- **Decision:** Version the config, report and selected-bundle schemas to v2 while preserving the verified `ghana-ocr-development-bundle-v1` archive contract. A configuration may compete only when it succeeds on every selected validation record with zero failures. A full report is selection-eligible only when every required engine has at least one complete, runtime-compatible configuration. Reject Tesseract versions outside major 5 with `OCR_ENGINE_VERSION_UNSUPPORTED`. Construct PP-OCRv6 CPU inference with `enable_mkldnn=false`. On Jammy, build Tesseract 5.5.3 only from official repository commit `db0ec62f81b0737fbbe184d8fea40af5738f8eef`, verify the installed major version and reuse the packaged English language data. Keep the v1 report/bundle hashes as historical failed-run evidence and reject them as current selection artifacts.
- **Consequences:** The prior EasyOCR `field_region` bundle is non-promotable and not a valid engine winner. The repaired notebook must rerun screen and clean validation at a new immutable commit before any current selected bundle exists. The locked test remains unopened, recognizer training remains false and the missing approved tampered-image slice still blocks robustness validation. Building Tesseract from source adds several minutes on a fresh Jammy runtime, but avoids silently accepting version 4 or trusting an unpinned PPA.
- **Related requirements/phases:** Logical PR17, PR19 and PR20; FR-OCR-001, FR-OCR-002, FR-OCR-003, NFR-ACC-001, NFR-PRIV-001, NFR-AUD-001, NFR-DATA-001, ADR-014, ADR-019, ADR-034, ADR-035.
- **Supersedes:** ADR-035's allowance to write an experimental selection from any measured validation candidate. It does not supersede the private development bundle, locked-test exclusion, raw-data privacy or missing-tampered-slice boundaries.

**2026-08-14 completed-validation addendum:** The repaired owner-operated run at immutable code commit `928f0cdae986428b747bd1839bb0a75d7d02404b` completed all 33 clean-validation records for one 100%-coverage finalist from each required engine with no adapter failure. PaddleOCR 3.7.0 `original_rgb` ranked first by the frozen weighted selector, ahead of EasyOCR 1.7.2 and Tesseract 5.5.3, but every exact-field gate failed and required-field parse success was `0.0`. Preserve report `55dc15e6aed31116fa4577f1135f6e92388b5baf951931c7a9a571c69ae94fb6` and selected bundle `ca38ece88260a5ff0760300a4921fc22c6745032e628cfc5bff2e182a2da2595` as valid clean-validation evidence with `experimental`, non-promotable status. Do not describe PaddleOCR as a production winner, do not open the locked test and do not train a recognizer.

**2026-08-14 attribution-diagnostic addendum:** OCR-output aggregates cannot identify whether exact-field loss originates in recognition or in the Ghana parser's supported text formats. Add a separate validation-only parser-ceiling report that runs the existing parser against human-verified transcripts and persists only aggregate field scores/counts and the inconclusive rate. It must not persist transcript text, normalized field values or record identifiers, and it must not initialize OCR engines, train, or load the locked test. Use this report to justify any subsequent parser change; do not tune from private examples by inspection or alter release gates to fit the result.

**2026-08-14 parser-ceiling result addendum:** The owner-operated diagnostic processed all 33 validation transcripts and wrote aggregate report `a7cb9a30225406d9677973d8da934d07b7b8be33de51d739a3225df21bc38967`. Exact extraction remained low without recognizer loss: amount `0.1875` over 32 scored records, recipient `0.03125` over 32, reference `0.05` over 20 and timestamp `0.0` over one. All 33 parses were inconclusive, and the sole record with all four truth fields failed. Treat parser/template coverage as the dominant measured bottleneck, but do not infer broad timestamp or complete-record performance from support of one. Before changing parser behavior, extend the same validation-only report with aggregate exact/unavailable/mismatch outcomes and stable warning counts; continue to exclude transcripts, values, record identifiers, OCR-engine initialization, recognizer training and locked-test access.

**2026-08-14 failure-taxonomy instrumentation addendum:** Version the aggregate parser ceiling as `ghana-ocr-parser-ceiling-report-v2`. For each truth-scored critical field, report exactly one of `exact`, `mismatch` or `unavailable`; continue to publish the scored denominator independently. Count critical-field parser warning codes only after they pass the bounded canonical uppercase-code pattern, and fail before writing on any malformed code so warning text cannot become a private-value channel. Do not change parser behavior from this instrumentation. The v1 report remains historical evidence until the v2 diagnostic executes on the same 33 validation transcripts; engine benchmarking, recognizer training and locked-test access remain prohibited for that run.

**2026-08-14 failure-taxonomy result addendum:** The owner-operated v2 diagnostic at immutable implementation commit `5576944c096240c701002b672914f1ddfc9b6bc1` processed the same 33 validation transcripts and wrote aggregate report `68bfd786359cd93095d2aa22384a823523b3ddf73915a220caca17db16b278e3`. Amount outcomes were 6 exact, 23 mismatch and 3 unavailable over 32 scored records; recipient 1/23/8 over 32; reference 1/13/6 over 20; and timestamp 0/0/1 over one. All 33 parses remained inconclusive. Because mismatch—not unavailability—dominates amount, recipient and reference, do not widen parser patterns or alter normalization based on these counts alone. First add a further aggregate-only validation diagnostic that attributes whether the expected value is present among normalized amount candidates or in normalized recipient/reference transcript containment, without persisting values, transcripts or identifiers. Timestamp remains deferred because its truth-scored support is one. Engine benchmarking, recognizer training and locked-test access remain prohibited.

## ADR-037 — Freeze parser v1 after aggregate mismatch attribution

- **Date:** 2026-08-15
- **Status:** Accepted
- **Decision owners:** Project owner and Codex/model steward under ADR-035/ADR-036
- **Context:** Owner-operated parser-ceiling v4 report `2405ea0e…` is canonically self-hash verified at immutable runtime `bf042c3a…`. Over 32 amount-scored records, 24 truths are absent from every valid amount candidate pool, two have no valid currency candidate and six are exact; neither active-pool selection-error category occurs. Recipient and reference are most often over-selected (`selected_contains_truth` at 15/32 and 10/20), but the aggregate-only privacy contract intentionally exposes no values or template examples from which to derive a safe termination rule. Timestamp and all-required support remain one record.
- **Options considered:** Widen or retune amount selection despite zero selection-error support; introduce recipient/reference trimming from private examples; add another private diagnostic with value-bearing examples; preserve parser v1 and move to the next governed product phase.
- **Decision:** Preserve `ghana-momo-parser-v1` unchanged and experimental. Do not tune parser behavior from private-value inspection, do not widen amount precedence, and do not claim accuracy improvement or deployability. Record the exact aggregate-only v4 evidence and begin logical PR18 analysis-product design from the final PR17 head with explicit partial/inconclusive OCR behavior.
- **Consequences:** PR17 closes with an auditable causal boundary rather than an inspection-driven parser patch. Amount candidate discovery/recognition coverage remains the largest measured gap; recipient/reference over-selection remains a documented limitation. PR19 must treat OCR/parser evidence as partial and experimental. Training and locked-test access remain prohibited until their governed phases.
- **Related requirements/phases:** Logical PR17-PR19; FR-OCR-001, FR-OCR-002, FR-OCR-003, NFR-ACC-001, NFR-PRIV-001, NFR-AUD-001, ADR-035, ADR-036.
- **Supersedes:** The open PR17 parser-change decision boundary only; it does not supersede the private development bundle, release gates, locked-test exclusion or missing tampered-slice blocker.

## ADR-038 — Use a categorical null-score policy until model evidence is accepted

- **Date:** 2026-08-15
- **Status:** Accepted
- **Decision owners:** Project owner and Codex under ADR-002/ADR-007
- **Context:** The product requirements retain configurable 40/40/20 image/structured/rule weights, but the only image training run failed acceptance at macro F1 `0.333333` and no screenshot-derived structured classifier is active. Averaging absent components would fabricate a probability and could turn a stored-reference match or weak deterministic heuristic into an unsupported low-risk claim.
- **Options considered:** Emit the configured weighted score with missing components treated as zero; renormalise available deterministic/rule evidence; block the complete product journey; use a versioned categorical precedence policy with null scores and explicit unavailable signals.
- **Decision:** PR18 uses hash-bound `analysis-risk-policy-demo-v1`. Critical stored-reference amount/reference mismatches may produce a categorical high-risk review band; accepted future structured/image models may supply versioned categorical bands under this contract. Scores remain null, deterministic image signals remain supporting-only, a stored-reference match never implies authenticity or low risk, and insufficient model evidence yields `PARTIAL`/`inconclusive`. Preserve the configured rule-set weights for a later calibrated policy version rather than silently changing or pretending to apply them.
- **Consequences:** The screenshot journey is usable and honest without activating rejected artifacts. FR-RISK-002/003 remain in progress until accepted validation evidence supports a separately versioned weighted/calibrated policy. Mobile/API/history must disclose model availability and never display a score when null.
- **Related requirements/phases:** Logical PR18-PR20; FR-RISK-001 through FR-RISK-007, FR-VER-007, FR-IMG-006, NFR-ACC-001.
- **Supersedes:** None; this is a bounded implementation of ADR-007 and a temporary compatibility decision under the fixed product taxonomy.

**2026-08-15 PR18 review addendum:** A categorical high-risk band does not convert unavailable required model evidence into a completed analysis. When either the accepted image or structured model is unavailable/failed, the persisted run remains `PARTIAL` while retaining the evidence-grounded band, null score, reasons and missing-signal disclosure. Cross-owner analysis, evidence and receipt access requires an investigator assigned to an active case; ADMIN/INVESTIGATOR role alone is insufficient. Migration `20260815_0003` makes analysis-stage rows immutable and prevents update/delete after an analysis reaches a terminal status. These controls preserve ADR-038's categorical policy while enforcing FR-RISK-005, FR-RISK-007 and object-scoped privacy at the database/API boundaries.

## ADR-039 — Ship PR19 as a local release with explicit degraded-model and dependency boundaries

- **Date:** 2026-08-16
- **Status:** Accepted
- **Context:** PR19 needs one reproducible product acceptance path across PostgreSQL, Flask, the staff portal and the Expo web release. The accepted image classifier does not exist, supported Expo/Metro dependencies retain upstream advisories, and this session has no hosted HTTPS environment or production credentials. A release claim cannot hide those facts.
- **Options considered:** Block all product acceptance; fabricate model availability; force unsupported mobile dependency downgrades; claim local containers as staging; deliver a local release with explicit partial-analysis, dependency and hosting boundaries.
- **Decision:** Package and verify the four services locally. Run migrations automatically, keep private storage behind authorised API routes, expose readiness per component, and serve a prebuilt mobile static export through Nginx. Keep image/structured model absence as degraded/not activated, never synthesize a success score, and record B-SEC-002 without `npm audit fix --force`. Treat model/rule operational pages as read-only until their higher-risk mutation contracts are implemented. Do not call the local HTTP environment hosted, staging or production.
- **Consequences:** The complete screenshot-to-case workflow can be demonstrated and regression-tested now. Non-local HTTPS, managed secrets, restore rehearsal, native/multi-browser and performance evidence, compatible dependency remediation, and accepted models remain explicit gates. The local mobile image requires a verified Expo web export before Compose build; `npm run infra:up` owns that ordering.
- **Related requirements/phases:** Logical PR19-PR20; FR-HIST-003 through FR-HIST-005, FR-CASE-001 through FR-CASE-006, FR-ADM-004 through FR-ADM-007, NFR-SEC-001/002, NFR-BACK-001, NFR-COMP-001.

## ADR-040 — Add obvious-scam OCR text evidence without replacing the categorical null-score policy

- **Date:** 2026-08-17
- **Status:** Accepted
- **Context:** Replacement implementation package `MoMo_OCR_Obvious_Fraud_Implementation_Package.zip` adds a useful deterministic Ghana MoMo scam-language assessment and immediate OCR preview, but its repository overlay targets the older PR17 tree and proposes a separate final risk projection. The current PR18/PR19 product already has hash-bound categorical policy, immutable analysis evidence and separate stored-reference verification. Replacing that policy would regress ADR-038 and risk presenting a rule score as a calibrated fraud probability.
- **Options considered:** Apply the package overlay verbatim; defer all text assessment until an optional trained model exists; add the deterministic evidence through the current semantic stage and policy; replace the current risk/verification contract.
- **Decision:** Port and harden the deterministic text assessor, its privacy-safe projection and the immediate mobile preview. Store the aggregate under the immutable OCR result and consume it in the existing `SEMANTIC_RULES` stage. Map `FRAUDULENT` text to high and `SUSPICIOUS` text to medium after critical stored-reference mismatches, while keeping the final policy score null. A no-match result has null class and never becomes `GENUINE`; missing required models keep the run `PARTIAL`; stored-reference verification remains independent. Historical OCR results without an assessment return an explicit unavailable state rather than being recomputed. Defer the package's optional text-model training module until its governed data and evaluation phase; do not access the locked test or add a disconnected model.
- **Consequences:** Users receive an actionable, accessible warning as soon as OCR completes, and the final analysis records versioned categorical text evidence without exposing raw matches. The integer text-rule score is useful only for rule ordering and is always marked `score_is_probability=false`. Existing API fields are extended additively, no database migration is required, and policy version advances to `analysis-risk-policy-demo-v2`. This change does not claim model accuracy, calibration, live MNO verification, deployment or production readiness.
- **Related requirements/phases:** FR-TEXT-RISK-001 through FR-TEXT-RISK-006, FR-OCR-002/005, FR-RISK-001/004/005, NFR-PRIV-001, NFR-AUD-001, NFR-ACC-001, logical PR18-PR20, ADR-038, ADR-039.
- **Supersedes:** None. It rejects only the replacement package's obsolete risk-projection overlay where it conflicts with the current higher-precedence product contracts.

## ADR-042 — Separate risk conclusiveness from component availability

- **Date:** 2026-08-17
- **Status:** Accepted
- **Context:** The categorical policy correctly retains `PARTIAL` when accepted image or structured-model components are unavailable, but finalization previously assigned `ANALYSIS_EVIDENCE_INCONCLUSIVE` to every partial run. Decisive OCR text or reference mismatch evidence could therefore produce a high band while API copy, notifications and reports called the result inconclusive. ADR-041 remains the proposed screenshot-only workflow decision and is not renumbered or silently accepted here.
- **Options considered:** Treat every `PARTIAL` run as inconclusive; mark decisive runs `COMPLETED` and hide missing components; remove terminal error semantics; model risk conclusiveness and component availability as independent additive projections.
- **Decision:** Preserve terminal execution status and immutable automated evidence. Add `conclusion_status` and `component_status` to policy, API and evidence projections. Low, medium and high bands are `CONCLUSIVE`; only the inconclusive band is `INCONCLUSIVE`. A conclusive `PARTIAL` run is `DEGRADED`, retains missing signals/limitations and uses `ANALYSIS_COMPONENTS_PARTIAL`; `ANALYSIS_EVIDENCE_INCONCLUSIVE` is reserved for the inconclusive band. Derive the same projection for historical runs rather than rewriting them. Result UI, notifications and reports lead with the risk conclusion and disclose degraded components second.
- **Consequences:** A high-risk warning is no longer semantically weakened by unavailable optional components, while component gaps remain visible and execution history stays compatible. The change is additive and requires no database migration. Clients consuming the generated OpenAPI schema receive required conclusion/component fields. PostgreSQL-backed integration and migration verification remains required once the local Docker engine is healthy.
- **Related requirements/phases:** Final-completion P0.1; FR-RISK-001, FR-RISK-004, FR-RISK-005, FR-RISK-007, FR-HIST-003, FR-HIST-004, NFR-AUD-001; ADR-038, ADR-040 and ADR-041.
- **Supersedes:** ADR-038's wording that coupled insufficient model evidence to `PARTIAL`/`inconclusive`; it does not supersede the `PARTIAL` execution status, categorical null-score policy, missing-signal disclosure, separate verification record or immutable-evidence boundary.

## ADR-041 — Persist screenshot-only text-risk analysis without fabricated transaction fields

- **Date:** 2026-08-17
- **Status:** Accepted
- **Context:** Obvious-scam message screenshots can contain decisive text evidence while omitting every field required for stored-record comparison. Requiring invented values breaks evidence integrity; keeping the warning transient breaks history, reporting and auditability.
- **Options considered:** Require fake confirmation values; return only a transient OCR warning; overload `UNVERIFIED`; run structured inference with defaults; add a versioned screenshot-only evidence mode.
- **Decision:** Add `analysis_mode` and an immutable OCR-result evidence link. Screenshot-only analysis skips verification and structured inference with `NOT_APPLICABLE_SCREENSHOT_ONLY`, persists verification `NOT_ATTEMPTED`, replays the stored versioned text assessment and uses available image evidence. Combined analysis retains its confirmation and stored-reference behavior. One `AnalysisEvidenceSelection` seam validates the mode-specific source before orchestration.
- **Consequences:** Message-only screenshots are reviewable in result, history, notification and private-report surfaces without raw OCR disclosure or fabricated features. Migration `20260817_0006` backfills historical rows to combined and forward-repairs B-MIG-001. API/OpenAPI/mobile changes are additive; no model is activated and no live-MNO claim is made.
- **Related requirements/phases:** Final-completion P0.2; FR-SCREENSHOT-001 through FR-SCREENSHOT-005; FR-TEXT-RISK-001/005/006; FR-VER-007; NFR-TEXT-PRIV-001; ADR-038, ADR-040, ADR-042.

## ADR-043 — Activate text ruleset v2 while retaining minimal v1 evidence validation

- **Date:** 2026-08-20
- **Status:** Accepted
- **Context:** P0.3 requires stronger Unicode, clause-locality, link/contact and classification behavior. Existing immutable OCR results already bind v1 classes, non-probabilistic scores and allowlisted reasons. Replacing the only accepted version would invalidate historical evidence; rerunning either ruleset against stored private OCR text would violate the replay and privacy contracts.
- **Options considered:** Rewrite v1 in place; discard v1 support and make prior evidence unavailable; expose both rulesets for new scans; activate v2 for new scans while retaining only the v1 classifier needed to validate stored safe projections.
- **Decision:** New OCR assessments use `ghana-momo-obvious-scam-rules-v2`, and the categorical policy advances to `analysis-risk-policy-demo-v3`. V2 removes Unicode `Cf` characters, applies contrastive and bounded adjacent-clause locality, admits scheme-less short links only with account/credential actions, handles ordinary/international contact redirects as a single high family, and classifies fraud only from one critical or two independent high families. The frozen v1 classifier remains internal solely for validation of stored v1 class/score/reason combinations. No public v1 selector or historical OCR recomputation is added.
- **Consequences:** New results gain the hardened behavior without changing historical classifications. A single high family plus urgency is now suspicious, while critical secret/payment/refund evidence and two independent high families remain fraudulent. Public and stored projections continue to reconstruct only allowlisted fixed copy and never expose matched values. The policy hash changes under a new policy version; no database or API-shape migration is required.
- **Related requirements/phases:** Final-completion P0.3; FR-TEXT-RISK-001 through FR-TEXT-RISK-006; NFR-TEXT-PRIV-001; NFR-TEXT-AUD-001; ADR-038, ADR-040, ADR-041 and ADR-042.

## ADR-044 — Freeze academic submissions from an exact pushed Git tree

- **Date:** 2026-08-21
- **Status:** Accepted
- **Context:** The repository contains safe controlled evidence alongside ignored private storage, local environment files, browser state, build outputs, model artifacts and research data. Copying the working directory would make the submitted revision ambiguous and could disclose excluded material. Committing a generated ZIP would duplicate the repository, make self-identification awkward and allow the derived artifact to drift from the branch it claims to represent.
- **Options considered:** Zip the working directory manually; commit a prebuilt submission archive; rely on `git archive` without a content manifest; build a deterministic ignored archive from an exact pushed Git tree with independent validation.
- **Decision:** `scripts/build_submission_package.py` is the supported freeze path. It requires a clean `HEAD` equal to its upstream, reads only `git archive <HEAD>`, rejects private/secret/generated/model/archive paths and size violations, validates complete safe evidence coverage, and writes a deterministic ZIP with a generated `FINAL_SUBMISSION_PACKAGE_MANIFEST.json`. The manifest records the exact commit and SHA-256/size of every source file. The ZIP and `.sha256` sidecar are written under ignored `output/submission/` and are not committed.
- **Consequences:** An examiner can identify and verify the exact submitted revision without trusting the mutable working tree. The artifact is reproducible from the pushed commit and cannot silently contain ignored private state. The generated manifest deliberately excludes its own hash to avoid recursion. Package verification proves archive integrity only; hosted deployment/CI, native-device acceptance, live MNO verification, model generalisation and outstanding product requirements remain separate evidence questions.
- **Related requirements/phases:** P20 final documentation/evidence freeze; NFR-DATA-001; NFR-SEC-002; NFR-PRIV-001; `12_FINAL_INSPECTION_PROTOCOL.md`; `13_DOCUMENTATION_AND_CHAPTER4_EVIDENCE.md`.
