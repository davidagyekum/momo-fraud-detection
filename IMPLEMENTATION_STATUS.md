...
- CI status: `Logical PR16 implementation gate passes locally with 428 tests at 90.78% branch-aware coverage; hosted jobs remain unable to start because the repository owner's Actions account is locked by a billing issue`
- Deployment status: `Not deployed`
- Current phase: `Logical PR16 private Ghana screenshot dataset — In Progress`
- Next exact task: `Complete manual de-identification and field/content annotation for the nine unique friend screenshots plus permitted online images 1, 2 and 4; crop/adjudicate online images 3 and 5 rather than assigning a whole-image fraud label. Keep image 6 rights-blocked.`
...
- Online boundary: six manually acquired images are privately indexed with zero exact/perceptual duplicates. Project-owner permission is attested for images 1-5; image 6 remains unpermitted. Content triage identifies three strong fraud examples, one ambiguous example and two mixed threads; none is training eligible before de-identification/adjudication. No scraper or bulk network acquisition client exists.
- Friend pilot: the project owner attested permission for ten newly supplied friend screenshots. Private intake produced ten records across eight conservative participant/source groups, quarantined one exact duplicate, left nine records pending de-identification and wrote zero working copies. Direct contributor forms were not supplied, so the evidence remains internal-only and non-training.
- Verification: the registered ML gate passes formatting, Ruff, strict mypy, 428 tests at 90.78% branch-aware coverage, governance, lock, notebook and controlled-data checks. No split, model fitting, metric or promotion claim is made.
...
| PR16-GHANA-PRIVATE | Logical PR16 | The private pipeline and initial friend pilot pass, but de-identification/content review remains. Site permission is owner-attested for online images 1-5, while image 6 remains unpermitted; only images 1, 2 and 4 have a strong whole-image fraud triage. | Ghana image/OCR adaptation and split freezing cannot start yet. | Keep Ghana-private disabled/non-training; retain raw screenshots, permission attestation and indexes outside Git; create no identifier-bearing working copies. | Project owner may later supply direct contributor/source evidence; Codex can perform de-identification and annotation now. | Produce reviewed de-identified derivatives, crop/adjudicate ambiguous/mixed candidates, then freeze groups only after training eligibility is explicitly approved. |
...
- Handoff file: `docs/handoffs/2026-08-13-PR16-ghana-private-pilot.md`
