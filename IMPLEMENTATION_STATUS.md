...
- Head SHA: `P10 functional commit pending; exact pushed head will be recorded in the handoff`
...
| P10 | Dataset governance, controlled sample generation and reproducible splits | Complete | `codex/p10-dataset-governance` — publication pending | Functional commit pending | Registered ML gate: 32 tests at 92.57% branch-aware coverage, Ruff, strict mypy, controlled-dataset validation and report-drift checks pass; 12 files across six isolated source groups reproduce manifest hash `51d12132…` and split hash `08008637…`; backend regression: 97 tests at 87.58% | No model was fit, evaluated or exported. FR-ML-005/006 remain In Progress until actual P11/P12 Colab runs record metrics and training provenance. |
