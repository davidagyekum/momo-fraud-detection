...
- CI status: `Logical PR16 implementation gate passes locally with 435 tests at 90.78% branch-aware coverage; hosted jobs remain unable to start because the repository owner's Actions account is locked by a billing issue`
...
- Online boundary: six manually acquired images are privately indexed with zero exact/perceptual duplicates. Project-owner permission is attested for images 1-5; image 6 remains unpermitted. Five metadata-stripped working copies now carry provisional labels—three fraud, one ambiguous and one mixed—and all require second review. No scraper or bulk network acquisition client exists.
- Friend pilot: the project owner attested permission for ten newly supplied friend screenshots. Private intake produced ten records across eight conservative participant/source groups and quarantined one exact duplicate. Nine metadata-stripped working copies now carry provisional labels—seven fraud, one genuine and one ambiguous. Direct contributor forms were not supplied, second review remains pending and every record is non-training.
- Verification: the registered ML gate passes formatting, Ruff, strict mypy, 435 tests at 90.78% branch-aware coverage, governance, lock, notebook and controlled-data checks. No split, model fitting, metric or promotion claim is made.
...
| PR16-GHANA-PRIVATE | Logical PR16 | Fourteen privacy-masked working copies and first-pass annotations exist, but independent privacy/label review remains. Site permission is owner-attested for online images 1-5, while image 6 remains unpermitted; two records are ambiguous and one is mixed. | Ghana image/OCR adaptation and split freezing cannot start yet. | Keep Ghana-private disabled/non-training; retain raw screenshots, permission attestation and indexes outside Git; expose only metadata-stripped derivatives to reviewers. | Project owner may later supply direct contributor/source evidence; a separate reviewer resolves the pending annotations. | Independently inspect privacy masks, crop/adjudicate ambiguous/mixed candidates, then freeze groups only after training eligibility is explicitly approved. |
