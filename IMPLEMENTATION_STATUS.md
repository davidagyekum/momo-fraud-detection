...
- Current phase: `Logical PR13 dataset acquisition and validation — In Progress; PaySim official archive acquired and hash-verified, pending Colab registration`
- Next exact task: `Push the PaySim registration notebook pin, run it in owner-operated Colab against the private Drive archive, and record the registered/quarantined manifest. Stop before splits, locked tests or training.`
...
- Honesty boundary: PaySim's official ZIP was downloaded, hashed and its one-member central directory inspected, but raw rows have not yet been validated and the dataset is not registered. No split, training, locked-test, metric or promotion claim is made.
...
- Traceability file last verified: `2026-08-11 — official PaySim Version 2 archive acquired to private Drive and content-addressed; Colab row validation remains pending, five sources remain blocked, and no training occurred`
...
| PR13-PAYSIM-COLAB | Logical PR13 | The official PaySim archive is acquired and hash-verified, but its 6.36M-row registration validator has not run in Colab. | PaySim remains disabled and `acquired_pending_registration`; PR14 cannot use it. | Preserve the private Drive archive and exact identity; do not infer row/schema/class validity. | Owner-operated Colab execution of the pinned PR13 notebook. | Run registration and record the safe manifest/profile; quarantine on any mismatch. |
