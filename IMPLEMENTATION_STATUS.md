...
- Message boundary: the owner's exact 2,654-message iMazing export is verified as incoming SMS from the authentic `MobileMoney` sender in the owner-iPhone backup. The fail-closed export quarantines 26 login/OTP/security-code messages and retains 2,628 raw rows. De-identification and exact collapse produce 116 texts across 90 template families: 52 transaction confirmations and 64 official-service messages. A complete manual privacy/utility pass found and corrected unusual greeting/reference name formats and independently approves all 116 de-identified rows. Residual direct-identifier/security-code pattern checks pass. Every row remains non-training and the corpus remains one owner lineage even though template groups also prevent format leakage.
...
- QA/split boundary: a private reviewed artifact records 129 approvals (116 owner-message and 13 screenshot-OCR rows) and zero exclusions at this text-review stage. All 219 real-plus-synthetic rows remain explicitly non-training. Split freezing still requires synthetic second review, explicit eligibility and at least 30 controlled-real groups; the current screenshot pilot has only 10. The owner corpus is one participant lineage and cannot supply independent evaluation groups. No split or training occurred.
- Verification: the registered ML gate passes formatting, Ruff, strict mypy, 486 tests at 90.01% branch-aware coverage, governance, lock, notebook and controlled-data checks. The secret/prohibited-artifact scan passes 527 candidates. Repository orchestration confirms the ML section passes but remains environment-blocked by inactive pinned Node/npm versions in the shared doctor; the security suite retains its already-documented unregistered marker. No split, model fitting, metric or promotion claim is made.
...
| PR16-GHANA-PRIVATE | Logical PR16 | Independent text review approves 116 owner-message and 13 screenshot-OCR rows; the balanced synthetic-clean pilot supplies 30 groups/90 rows but still requires its second review. The screenshot pilot remains only 10 controlled-real groups versus the enforced 30-group minimum, and image 6 remains unpermitted. | Ghana split freezing and adaptation cannot start; no current record is training eligible. | Keep Ghana-private disabled/non-training and preserve the audited private indexes outside Git. | Project owner/data steward expands independent, permission-attested controlled-real groups and reviews the synthetic pilot. | Add at least 20 controlled-real source groups with stronger genuine/suspicious coverage, review all synthetic rows, then explicitly enable eligible records and freeze group-safe splits before Colab training. |
...
- Handoff file: `docs/handoffs/2026-08-14-PR16-text-review-synthetic-pilot.md`
- Summary: `Hardened and independently reviewed the private owner/screenshot text corpora, quarantined all detected security-code messages, generated a balanced 30-group synthetic-clean pilot, and stopped with every record non-training and all splits unfrozen.`
...
8. Preserve the completed second-review artifact for 129 real de-identified text rows and the 30-group synthetic-clean pilot. Review the 90 synthetic records and expand controlled-real screenshot groups to at least 30 before any eligibility or split-freeze decision. No raw private data, consent record or downloaded image may enter Git.
