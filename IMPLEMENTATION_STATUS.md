...
- CI status: `Logical PR16 ML gate passes locally with 525 tests at 90.05% branch-aware coverage; the repository orchestrator remains subject to the documented inactive Node/npm toolchain mismatch. Hosted jobs remain unable to start because the repository owner's Actions account is locked by a billing issue.`
- Deployment status: `Not deployed`
- Current phase: `Logical PR16 private Ghana screenshot dataset — In Progress`
- Next exact task: `Obtain at least 17 additional permission-attested controlled-real source groups with preserved participant-to-file mapping, prioritising suspicious examples. Then make an explicit eligibility decision, freeze group-safe splits only if readiness passes, and stop before Colab training.`
...
- OCR/text boundary: 42 label-approved screenshot records now have manually verified OCR truth stored privately: the prior 20 plus 22 newly unique genuine friend screenshots. The new records explicitly use coarse full-image localization, not field-localization ground truth. Typed-placeholder de-identification replaces declared direct values while preserving transaction language. Independent text review approves all 42 rows with zero residual sensitive-field or malformed-placeholder findings. Image derivatives, including earlier masks, remain excluded from training; private originals are evidence/OCR sources only.
...
- QA/split boundary: a private reviewed artifact records 397 approvals: 106 iPhone owner-message, 159 Android owner-message, 42 screenshot-OCR and 90 corrected synthetic rows. Its safe readiness report counts 13 controlled-real screenshot groups versus the required 30 and 30 synthetic groups versus the required 20. The owner accounts remain one train-only participant lineage regardless of 222 template groups; the unmapped friend addition contributes one group. `ready_to_freeze=false`, every row remains non-training and no split or model run exists.
- Verification: the registered ML gate passes formatting, Ruff, strict mypy, 525 tests at 90.05% branch-aware coverage, governance, lock, notebook and controlled-data checks. The secret/prohibited-artifact scan passes 531 candidates. Repository orchestration remains subject to the previously documented inactive pinned Node/npm versions in the shared doctor; the security suite retains its already-documented unregistered marker. No split, model fitting, metric or promotion claim is made.
