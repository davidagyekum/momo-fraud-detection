...
- OCR/text boundary: 20 label-approved screenshot records now have exact, human-corrected OCR truth stored privately: the prior 13 plus seven newly approved Ghana fraud screenshots. Typed-placeholder de-identification preserves spelling, sender context and blocking/reversal language while replacing direct values. Independent text review approves all 20 rows with zero residual sensitive-field or malformed-placeholder findings. Image derivatives, including earlier masks, remain excluded from training; private originals are evidence/OCR sources only.
...
- QA/split boundary: a private reviewed artifact records 375 approvals: 106 iPhone owner-message, 159 Android owner-message, 20 screenshot-OCR and 90 corrected synthetic rows. Its safe readiness report counts 12 controlled-real screenshot groups versus the required 30 and 30 synthetic groups versus the required 20. The owner accounts remain one train-only participant lineage regardless of 222 template groups. `ready_to_freeze=false`, every row remains non-training and no split or model run exists.
- Verification: the registered ML gate passes formatting, Ruff, strict mypy, 519 tests at 90.11% branch-aware coverage, governance, lock, notebook and controlled-data checks. The secret/prohibited-artifact scan passes 530 candidates. Repository orchestration remains subject to the previously documented inactive pinned Node/npm versions in the shared doctor; the security suite retains its already-documented unregistered marker. No split, model fitting, metric or promotion claim is made.
...
| PR16-GHANA-PRIVATE | Logical PR16 | Independent text review approves 265 owner-account, 20 screenshot-OCR and 90 corrected synthetic rows. The synthetic 30-group gate passes, but only 12 controlled-real screenshot groups exist versus the enforced 30. The owner accounts remain one train-only participant lineage and cannot fill the independent-group gate; image 6 remains unpermitted. | Ghana split freezing and adaptation cannot start; no current record is training eligible. | Keep Ghana-private disabled/non-training, preserve one conservative lineage for the latest online batch and keep private artifacts outside Git. | Project owner/data steward supplies at least 18 additional permission-attested controlled-real source groups with stronger genuine/suspicious coverage. | Reach at least 30 controlled-real groups, explicitly enable approved records, freeze leakage-safe partitions and then stop for owner confirmation before Colab training. |
...
- Handoff file: `docs/handoffs/2026-08-14-PR16-permitted-batch-text-review.md`
- Summary: `Recorded permission for exactly 11 candidates, approved 7 and excluded 4 through OCR-first review, completed the corrected synthetic review, and stopped at the 12/30 controlled-real group gate without enabling, splitting or training.`
...
8. Preserve the completed 375-row reviewed text artifact, the corrected 30-group/90-row synthetic v3 corpus and the 20-row screenshot OCR layer. The readiness report remains blocked at 12/30 controlled-real groups; obtain at least 18 more independent permission-attested groups with stronger genuine/suspicious coverage before eligibility or split freezing. No raw private data, consent record or downloaded image may enter Git, and stop before any Colab training.
