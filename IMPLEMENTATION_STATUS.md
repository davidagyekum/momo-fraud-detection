...
- CI status: `Logical PR16 ML gate passes locally with 486 tests at 90.01% branch-aware coverage; the repository orchestrator reports only the documented inactive Node/npm toolchain mismatch. Hosted jobs remain unable to start because the repository owner's Actions account is locked by a billing issue.`
- Deployment status: `Not deployed`
- Current phase: `Logical PR16 private Ghana screenshot dataset — In Progress`
- Next exact task: `Independently review the 90 synthetic-clean records, expand the controlled-real pilot from 10 to at least 30 permission-attested source groups, then make an explicit eligibility decision. Only after those gates pass may group-safe splits be frozen; stop and notify the owner before Colab training.`
...
- Online boundary: the initial six manually acquired images remain as previously reviewed: permission is attested for images 1-5, image 6 remains unpermitted, three whole-image fraud labels are approved, one ambiguous image is excluded and the mixed image is superseded by suspicious/genuine controlled crops. A later owner-supplied batch adds 11 exact/perceptually unique quarantined leads: seven primary Ghana MoMo fraud candidates, two adjacent Ugandan mobile-money phishing examples and two authenticity-ambiguous cash-in screenshots. All 11 expose identifiers and lack recorded source-page/permission evidence, so they add zero controlled-real groups, have no working derivatives and remain non-training. No scraper or bulk network acquisition client exists.
...
- OCR/text boundary: all 13 label-approved screenshot records have exact, human-corrected OCR truth stored privately. The corrected de-identification pass uses a single original-text substitution so short field values cannot corrupt typed placeholders. Independent text review approves all 13 rows with zero exact sensitive-field or malformed-placeholder findings. Image derivatives, including earlier masks, remain excluded from training; private originals are evidence/OCR sources only.
- Synthetic-clean boundary: a deterministic private pilot now contains 30 immutable source groups and 90 wholly fictitious text records, balanced at 30 `FRAUDULENT`, 30 `GENUINE` and 30 `SUSPICIOUS`. It exceeds the 20-group synthetic minimum but remains pending second review, non-training and unfrozen; it is supplementary controlled data, not evidence of real prevalence or performance.
- QA/split boundary: a private reviewed artifact records 278 approvals (106 iPhone owner-message, 159 Android owner-message and 13 screenshot-OCR rows) and zero exclusions at this text-review stage. All 368 real-plus-synthetic rows remain explicitly non-training. Split freezing still requires synthetic second review, explicit eligibility and at least 30 controlled-real groups; the current screenshot pilot has only 10. Provider/account/template diversity improves training coverage but does not create independent participants. No split or training occurred.
- Verification: the registered ML gate passes formatting, Ruff, strict mypy, 506 tests at 90.14% branch-aware coverage, governance, lock, notebook and controlled-data checks. The secret/prohibited-artifact scan passes 529 candidates. Repository orchestration remains subject to the previously documented inactive pinned Node/npm versions in the shared doctor; the security suite retains its already-documented unregistered marker. No split, model fitting, metric or promotion claim is made.
...
| PR16-GHANA-PRIVATE | Logical PR16 | Independent text review approves 265 owner-account texts plus 13 screenshot-OCR rows; MTN/Telecel/account diversity does not create independent participants. The balanced synthetic-clean pilot supplies 30 groups/90 rows but still requires second review. The screenshot pilot remains only 10 controlled-real groups versus the enforced 30-group minimum. Image 6 and the latest 11 unique quarantined leads lack recorded permission/source evidence; two new leads also require authenticity adjudication. | Ghana split freezing and adaptation cannot start; no current record is training eligible. | Keep all owner accounts train-only candidates, Ghana-private disabled/non-training and private indexes outside Git. | Project owner/data steward supplies source/permission evidence for usable new candidates and reviews the synthetic pilot. | Resolve rights and authenticity for the latest candidates, reach at least 30 distinct controlled-real source groups, review all synthetic rows, then explicitly enable eligible records and freeze group-safe splits before Colab training. |
...
- Handoff file: `docs/handoffs/2026-08-14-PR16-suspicious-online-batch.md`
- Summary: `Privately quarantined and conservatively triaged 11 unique suspicious-message leads, preserved all rights/authenticity blockers, and stopped without derivatives, training eligibility or split changes.`
...
8. Preserve the completed second-review artifact for 278 real de-identified text rows, the 30-group synthetic-clean pilot and the 11 newly quarantined online leads. Resolve source-page/permission evidence and two authenticity adjudications before treating any new lead as a controlled-real group. Review the 90 synthetic records and reach at least 30 controlled-real groups before any eligibility or split-freeze decision. No raw private data, consent record or downloaded image may enter Git.
