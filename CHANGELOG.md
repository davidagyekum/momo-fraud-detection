...
- Added the logical PR16 private Ghana screenshot foundation: owner/friend permission-state intake, pseudonymous participant/source groups, exact/perceptual duplicate quarantine, auditable review transitions, withdrawal propagation, group-safe split freezing, online-candidate rights quarantine and an output-free Colab dataset notebook. The current 13 label-approved screenshot records were converted into exact private OCR truth, a raw private CSV and a matching text-only de-identified CSV. Sensitive values are replaced with typed placeholders instead of masking image pixels; automated comparison found zero exact sensitive-field leaks. All image derivatives are excluded from training, every CSV row remains pending second review and non-training, and the 30 controlled-real/20 synthetic-clean minimum still blocks split freezing. No split or training occurred.
- Exported the owner's verified 2,654 incoming `MobileMoney` iMazing messages to a raw private CSV and 230 exact-deduplicated de-identified review rows across 167 template families. The text sanitizer replaces counterparties, phones, amounts, references, emails and URLs while preserving message wording; aggregate QA found zero residual direct-identifier pattern hits. All owner messages remain one private owner lineage, pending second review and non-training; no split or model run occurred.
