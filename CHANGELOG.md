...
- Hardened the validation-only PR17 parser-ceiling v4 mismatch-attribution contract at immutable runtime commit `bf042c3a…` and repinned it in the output-free owner notebook at `8afac88b…`. Raw labelled-source activation is now independent from valid pool presence, private JSON/hash/write failures are generic and unchained, output writes are atomic, anchored references stop before prose, and recipient evidence removes only surrounding punctuation. Parser outputs are unchanged, `RUN_BENCHMARK=False` remains set, and the owner instruction is: first four code cells, ending with parser-ceiling; stop before adapter code cell. No private v4 aggregate, locked-test access or training is claimed. The local ML gate passes 714 tests at 90.15% coverage and the 560-file secret/artifact scan passes; the repository wrapper remains non-zero only for documented host-tool mismatches.
