...
- Added a validation-only, aggregate-redacted parser-ceiling diagnostic to separate parser-format limitations from OCR-recognition loss. The report records only field scores/counts and parser inconclusive rate; it cannot persist private transcripts, field values or record identifiers and does not initialize OCR engines or access the locked test.
