...
- Aligned the PR17 parser-ceiling measurement contract so exact scoring, availability classification and warning attribution all consume one explicit in-memory field comparison. Recipient-name truth uses the name parser field and recipient-wallet truth uses the wallet field; v3 reports add bounded subtype, secondary-truth and observed-field warning aggregates while persisting no values or identifiers. Conflicting duplicate truth and missing required parser fields now fail closed. Parser behavior, OCR engines, release gates, training and locked-test access remain unchanged; the private 33-record v3 validation run is still owner-operated and pending.
