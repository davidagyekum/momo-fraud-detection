...
- Versioned the parser-ceiling report to v2 with per-field `exact`/`mismatch`/`unavailable` counts and aggregate critical-field warning-code counts. Warning keys must match a bounded canonical identifier or the report fails before writing, preventing values from entering the aggregate channel. Parser behavior, OCR engines, training and locked-test access remain unchanged; private v2 execution is pending the immutable Colab pin.
