...
- Implemented the validation-only PR17 parser-ceiling v4 mismatch-attribution contract at immutable runtime commit `22a30a5a…` and pinned it in the output-free owner notebook at `9ac6d4db…`. The additive `ghana-ocr-mismatch-attribution-v1` report uses only fixed aggregate candidate-pool, bounded-count and field-attribution categories; exact identity, allowlist, denominator, privacy and self-hash validation fails closed before atomic output. Parser outputs are unchanged, `RUN_BENCHMARK=False` remains set, owner execution stops after cells 1-4 before adapters, and no private v4 aggregate, locked-test access or training is claimed. The local ML gate passes 704 tests at 90.08% coverage and the final 560-file secret/artifact scan passes; the repository wrapper remains non-zero only for documented host-tool mismatches.
